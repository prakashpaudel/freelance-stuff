...
/**
 * Sans Quote Post Format Loop Template Part
 *
 * This template contains the quote post format loop template part.
 *
...
// store the title & options in variables for conditional testing
$title = get_the_title();
$quote = get_post_meta( $post->ID, '_wap8_quote_full', true );
$cite = get_post_meta( $post->ID, '_wap8_quote_author', true ); ?>
<article id="post-<?php the_ID(); ?>" <?php post_class( 'sans-blog-post sans-blog-img' ); ?>>
	<section class="sans-entry sans-entry-<?php the_ID(); ?>">
		...
		<?php
			echo '<blockquote>';
				echo '<p>&#147;' . $quote . '&#148;</p>';
				if ( $cite ) { echo '<p class="quote-source">' . $cite . '</p>'; }
			echo '</blockquote>';
		?>
